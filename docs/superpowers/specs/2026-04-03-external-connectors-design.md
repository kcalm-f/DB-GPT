...
DB-GPT 的 ReAct Agent 已经能够执行数据分析、SQL 取数、代码执行和报告生成等任务。但当前分析结果只能在平台内查看或通过分享链接共享，无法自动自动到外部服务。

本方案提出**“技能+MCP双轮驱动”**架构，使DB-GPT Agent能够：
- 从外部服务拉取数据作为分析输入（语雀文档、飞书表格、GitHub Issues 等）
- 将分析结果自动生成到外部服务（钉钉群通知、语雀文档、飞书消息、邮件等）
- 支持L3级别的定时分析+主动（如“每天早上8点执行销售分析，发布到语雀，通知钉钉群”）
...
└──────────────────────────────────────────┼────────────────────┘
         ...
         └──────┘  └──────┘  └──────┘  └──────┘  └──────┘
```
### 2.3 为什么不是原生 Python？ (全MCP统一方案)

**早期方案（已废弃）** 曾考虑中国服务使用原生Python实现、海外服务使用MCP。经调研发现：

| 服务 | MCP 服务器 | 来源 | 状态 |
|------|------------|------|------|
| 飞书 | 飞书 | `larksuite/lark-openapi-mcp` | **官方** | 稳定|
| 钉钉 钉钉 | 钉钉`open.dingtalk.com` MCP 服务器 | **官方** | 稳定|
| 语雀语雀| `语雀-mcp` (PyPI) | 社区 | 可用 |
| 企微 微信工作 | `wecom-bot-mcp-server` (PyPI) | 社区 | 可用 |
| GitHub | `@modelcontextprotocol/server-github` | 官方| 稳定|
|松弛| `@modelcontextprotocol/server-slack` | 官方| 稳定|
|谷歌表格 | `@anthropic/google-sheets-mcp` | 社区 | 可用 |
|概念 | `@notionhq/notion-mcp-server` | 官方| 稳定|
|电子邮件 | SMTP MCP 服务器 | 社区 | 可用 |

**结论**：所有目标服务均已实现MCP Server实现（2026年mcp.so已收录19,400+台服务器），DB-GPT消耗自行编写任何外部服务API调用代码。

**完整MCP统一方案的优势**：
1. **零维护成本**：API更新由MCP Server社区/官方维护
2. **标准化**：统一的MCP协议，DB-GPT端点集合连接管理代码
3. **扩展性**：新服务=新的MCP服务器配置边界，支出写入代码
4. **生态复用**：直接复用19,400+社区MCP服务器

### 2.4 与行业比较

| 维度 |马努斯 |法典| DB-GPT（本方案）|
|------|--------|--------------------|----------------|
| 连接器协议 | MCP + 哺乳期 | MCP（OpenAI 生态）| MCP统一|
| 认知层 | 内置工作流程 |提示|技能（SKILL.md）|
| 交互模式 | 面板式+自动| 配置文件 | 面板式+自动+手动触发|
| 安全确认 | 部分有 | 无 | **连接器写操作确认+定时任务事后通知** |
| 定时任务 | 无 | 无 | **L3 Scheduler（核心差异）** |
| 数据分析集成 | 弱| 无 | ** 纯粹的 SQL/代码分析 + 外推** |
| 差异化 | 通用AI代理| 代码助手 | **数据分析 + 外部协作** |
...
ConnectorManager 是连接器的核心设备，负责备用管理、MCP 服务器生命周期和工具注入。
```python
...
> **原则**：原有ReAct Agent的技能、内置工具执行流程不出行。仅对新增的MCP连接器写入操作加入确认机制。

确认逻辑在**ConnectorManager注入的工具封装层**实现，不在Agent核心管道中：
```python
...
- **定时任务触发**：自动授权默认免确认→执行后自动通知（见9.3节）
- **原有代理工具**：不出行，无拦截拦截
...
**关键文件文件**: `packages/dbgpt-app/src/dbgpt_app/openapi/api_v1/agentic_data_api.py`
...
```
### 4.3 通过 required_tools 进行技能集成

技能通过 `required_tools` 声明连接器依赖，使代理知道该技能有哪些外部服务：
```yaml
...
```
当代理加载此技能时，会自动检查`required_tools`中的连接器是否已配置。如果用户未连接语雀或钉钉，代理提示用户先配置连接。
...
1. **scan_serve_configs()** — 在 `packages/dbgpt-app/src/dbgpt_app/initialization/serve_initialization.py` 的模块列表中添加：
   ```python
   modules = [
       ...
...
2. **register_serve_apps()** — 在同一文件的注册函数中添加条件注册块：
   ...
   ```
3. **config.py标准常量** — `packages/dbgpt-serve/src/dbgpt_serve/connector/config.py`：
   ```python
   APP_NAME = "connector"
...
|路径|目的|
|------|---------|
| `packages/dbgpt-core/src/dbgpt/agent/resource/connector/__init__.py` |连接器模块|
| `packages/dbgpt-core/src/dbgpt/agent/resource/connector/manager.py` |连接器管理器 |
| `packages/dbgpt-core/src/dbgpt/agent/resource/connector/catalog.py` | ConnectorCatalog（JSON 配置加载器）|
| `packages/dbgpt-core/src/dbgpt/agent/resource/connector/confirmation.py` |确认拦截器 |
| `packages/dbgpt-ext/src/dbgpt_ext/connector/catalog.json` |连接器目录数据|
| `packages/dbgpt-serve/src/dbgpt_serve/connector/` |整个服务模块|
| `web/pages/construct/connectors/index.tsx` |连接器管理页面|
| `网络/组件/连接器/` |连接器 UI 组件 |

### 修改文件
|路径|改变 |
|------|--------|
| `packages/dbgpt-app/.../agentic_data_api.py` |在 `_react_agent_stream()` 中注入连接器工具 |
| `packages/dbgpt-app/.../component_configs.py` |注册ConnectorManager组件|
| `packages/dbgpt-core/.../agent/resource/base.py` |将 `Connector` 添加到 ResourceType 枚举 |
| `web/pages/index.tsx` |在聊天页面添加连接器状态指示器 |
| `packages/dbgpt-app/.../serve_initialization.py` |在 `scan_serve_configs()` 和 `register_serve_apps()` 中注册 ConnectorServe |
...
- 服务模块（API + DB模型）
- 人在环确认机制
- 前 3 个连接器：飞书（官方 MCP）、钉钉（官方 MCP）、语雀（社区 MCP）
- 前端连接器管理页面（基本）

### 第 2 阶段：技能增强（2 周）
- 使用连接器工具解决技能“required_tools”问题
- 系统提示动态连接器描述注入
- 加载技能时自动检查连接器可用性
- 增强了连接器操作的步骤 UI
...
### 第 3 阶段：L3 调度程序（3 周）
- 定期分析+推送的定时任务系统
- **定时任务默认自动授权**：`trigger_type=scheduled` 时ConfirmationInterceptor 免确认
- **执行后通知**：全民执行结果通知给用户（含成功/失败状态 + 回退入口）
...
### 第 4 阶段：生态系统扩展（正在进行）
- 其他连接器：企微、GitHub、电子邮件、Notion、Slack、Google Sheets
- 需要 OAuth 2.0 服务的流程
- 连接器市场（社区贡献）
- 高级功能：webhook 触发器、事件驱动的工作流程
...
### 端到端测试场景
1. **输出**：用户说“分析数据并发布到语雀”→ Agent执行分析→调用`yuque_create_doc`→用户确认→文档出现在语雀中
2. **输入**：用户说“读取飞书文档分析”→ Agent调用`feishu_get_doc`→处理内容→返回分析
3. **双向**：用户说“读取语雀目标文档，对比SQL数据，分析报告发到钉钉”→全输入+分析+输出流程
4. **连接管理**：用户在前端添加语雀令牌→开始聊天→代理自动发现并使用语雀工具
5. **安全**：数据库存储加密凭证；写操作需要确认；读操作直接执行
6. **定时任务**：Cron触发分析→自动确认（无确认UI）→yuque_create_doc + dingtalk_send_message→用户收到执行摘要通知+回滚链接→用户点击回滚→执行补偿MCP调用
7. **回滚**：定时推送后，用户点击回滚→系统调用`yuque_delete_doc(doc_id)`→验证doc已删除→标记回滚完成

### 单元测试
- ConnectorManager 生命周期测试
- CredentialStore加密/解密测试
- ConnectorCatalog 加载和验证测试
- 确认拦截器逻辑测试
- API端点CRUD测试