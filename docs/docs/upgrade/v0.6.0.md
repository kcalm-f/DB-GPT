...
本指南适用于从 v0.5.10 升级到 v0.6.0。如果您使用SQLite，则无需升级数据库。如果使用MySQL，则需要升级数据库。
...
为防止数据丢失，建议升级前备份数据库。备份方式根据您的数据库类型而定。
...
:::提示
版本升级所需的资源文件放在[此处](https://github.com/eosphoros-ai/DB-GPT/tree/main/assets/schema/upgrade)。
:::
```sql
USE dbgpt;
-- chat_history
